...
bool MMC5883MA::setup() {
    ...
    i2c_write_to_subaddr(REGISTERS::INTERNAL_CONTROL_2,0x00);
    i2c_write_to_subaddr(REGISTERS::INTERNAL_CONTROL_1,0x03);
    i2c_write_to_subaddr(REGISTERS::INTERNAL_CONTROL_0,0x03);
    return i2c_ping();
}
...
int16_t MMC5883MA::get_temp() {
    Serial.printf("Is temp measurement happening: %d\n", i2c_read_from_subaddr(REGISTERS::TEMPERATURE));
    return i2c_read_from_subaddr(REGISTERS::TEMPERATURE);
}
